...
# Make three-ish commands :
# One command will be for running `identify_duplicates_in_a_list.py` on each list
# , one command to add the standad gene name to the output, and then a couple of
# commands at end to make a directory and copy all the files from earliery
# running to it based on the tag in the name.
...
for list_file in lists_of_list_files:
    ...
    file_name_expected = generate_output_file_name_function_from_duplicate_fiding_script(
        list_file, file_name_tag_to_use_for_moving)
...
if just_output_commands_only:
    ...
    # script, I cannot be sure if a output file made or not and so producing
